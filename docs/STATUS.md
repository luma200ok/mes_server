...
### P0 — 스키마 관리 기반 (모든 스키마 변경의 선행)
- [ ] **Flyway 도입 + `mes_db` baseline** — 현재 prod `ddl-auto: update` 로 마이그레이션 이력 없음
- [ ] `ddl-auto` → `validate` (local/prod 전부), `out-of-order: true`
...
| — | prod `ddl-auto: update` — 스키마 롤백 불가 | 미해결 |
